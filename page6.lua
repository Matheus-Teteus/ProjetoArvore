...
local function abrirWelcome(self, event)
    if event.phase == "ended" or event.phase == "cancelled" then
        composer.gotoScene("page5", "slideRight", 800)
        return true
    end
end
...
function scene:create(event)
    ...
    local background = display.newImageRect(sceneGroup, "assets/fundo02.png", display.contentWidth, display.contentHeight * 2)
    background.anchorX = 0
    background.anchorY = 0
    background.x, background.y = 0, -350

    local imginfo = display.newImageRect(sceneGroup, "assets/cidade.png", display.contentWidth*1.1, display.contentHeight * 0.8)
    imginfo.anchorX = 0
    imginfo.anchorY = 0
    imginfo.x, imginfo.y = -16, 430

    local options1 = {
        text = "A preservação do meio ambiente começa com pequenas atitudes diárias, que fazem toda a diferença. Uma das mais importantes é a reciclagem do lixo.!",
        x = display.contentWidth * 0.5,
        y = display.contentHeight - (display.contentHeight * 1),
        width = 600,
