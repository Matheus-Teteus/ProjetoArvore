...
local function abrirWelcome(self, event)
    if event.phase == "ended" or event.phase == "cancelled" then
        composer.gotoScene("page4", "slideRight", 800)
        return true
    end
end
...
local function abrirPage2(self, event)
    if event.phase == "ended" or event.phase == "cancelled" then
        composer.gotoScene("page6", "slideLeft", 800)
        return true
    end
end
...
function scene:create(event)
    ...
    local background = display.newImageRect(sceneGroup, "assets/fundo03.png", display.contentWidth, display.contentHeight * 2)
    background.anchorX = 0
    background.anchorY = 0
    background.x, background.y = 0, -150

    local imginfo = display.newImageRect(sceneGroup, "assets/planeta3.png", display.contentWidth*1, display.contentHeight * 0.8)
    imginfo.anchorX = 0
    imginfo.anchorY = 0
    imginfo.x, imginfo.y = 0, 600

    local options1 = {
        text = "Preservar e plantar uma árvore,é olhar para o futuro!",
        x = display.contentWidth * 0.5,
        y = display.contentHeight - (display.contentHeight * 0.8),
        width = 600,
