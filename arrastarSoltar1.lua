...
local arvore1 = display.newImageRect(grupo, imageSheetA ,1,400, 400)
arvore1.x = _W /2
...
local mudinha = display.newImageRect(grupo, imageSheetM, 1, 100, 130)
mudinha.x = _W / 5;
mudinha.y = _H * 0.6;
...
local MMusic2 = audio.loadStream("assets/somserra.mp3")
...
function arrastar(e)
  ...
  if (phase == "began") then
    ...
    audio.play( MMusic2, { channel=2, loops=0 } )
...
function arrastarPlanta(e)
  ...
  elseif (phase == "ended") then
    stage:setFocus(body, nil)
    body.tempJoint:removeSelf()
    local arvore3 = display.newImageRect(grupo, imageSheetM ,1,80, 80)
    arvore3.x = e.x
    arvore3.y = e.y
